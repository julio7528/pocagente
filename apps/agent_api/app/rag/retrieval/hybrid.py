...
class HybridRetriever:
    ...
    async def search(self, query: str) -> Sequence[RetrievedChunk]:
        ...
        if self._database is None:
            lexical = await self._lexical.search(query, self._config.lexical_candidate_limit)
            semantic = await self._semantic.search(query, self._config.semantic_candidate_limit)
        ...
        async with self._database.transaction() as connection:
            ...
            lexical = await LexicalRetriever(repository).search(
                query, self._config.lexical_candidate_limit
            )
            semantic = await SemanticRetriever(
                repository, self._semantic.embed_adapter
            ).search(query, self._config.semantic_candidate_limit)
            return tuple(self._ranker.rank(lexical, semantic, self._config.final_top_k))
