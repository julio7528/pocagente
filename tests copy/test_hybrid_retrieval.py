...
def test_channel_retrievers_validate_and_delegate() -> None:
    ...
    async def run() -> None:
        assert len(await lexical.search("query", 10, KnowledgeScope.INTERNAL)) == 1
        assert len(await semantic.search("query", 10, KnowledgeScope.INTERNAL)) == 1
        with pytest.raises(ValueError):
            await lexical.search(" ", 10, KnowledgeScope.INTERNAL)
        with pytest.raises(ValueError):
            await semantic.search(" ", 10, KnowledgeScope.INTERNAL)

    asyncio.run(run())
    repository.search_lexical_candidates.assert_awaited_once_with("query", 10, KnowledgeScope.INTERNAL)
...
def test_hybrid_public_api_fuses_two_channels_and_uses_config_limits() -> None:
    ...
    async def run() -> None:
        result = await retriever.search("query", KnowledgeScope.INTERNAL)
        assert len(result) == 1
        assert result[0].chunk.chunk_id == 1

    asyncio.run(run())
    lexical.search.assert_awaited_once_with("query", 3, KnowledgeScope.INTERNAL)
    semantic.search.assert_awaited_once_with("query", 4, KnowledgeScope.INTERNAL)
...
def test_hybrid_snapshot_uses_one_transaction_and_same_repository_connection(monkeypatch) -> None:
    ...
    asyncio.run(retriever.search("snapshot", KnowledgeScope.INTERNAL))
    ...
    repository.search_lexical_candidates.assert_awaited_once_with("snapshot", 10, KnowledgeScope.INTERNAL)
