...
async def test_lexical_retrieval_uses_approved_fts_and_active_filters() -> None:
    ...
    candidates = await repository.search_lexical_candidates("Falha R1", 10, KnowledgeScope.INTERNAL)
    ...
    assert query.index("s.origin") < query.index("LIMIT %s")
    assert "PUBLIC_OFFICIAL" in statement.parameters
    assert "ORDER BY channel_score DESC, chunk_id ASC" in query
    assert "LIMIT %s" in query
    assert statement.parameters[-1] == 10
    assert "ops." not in query and "audit." not in query
...
async def test_lexical_retrieval_rejects_blank_query_or_unbounded_limit() -> None:
    ...
    with pytest.raises(ValueError, match="blank"):
        await repository.search_lexical_candidates("   ", 5, KnowledgeScope.INTERNAL)
    with pytest.raises(ValueError, match="between 1 and 10"):
        await repository.search_lexical_candidates("query", 11, KnowledgeScope.INTERNAL)
...
async def test_semantic_retrieval_uses_exact_cosine_and_active_filters() -> None:
    ...
    candidates = await repository.search_semantic_candidates(embedding, 7, KnowledgeScope.INTERNAL)
    assert len(candidates) == 1
    assert isinstance(candidates[0], SearchCandidate)
...
async def test_semantic_retrieval_rejects_wrong_dimension_or_limit_before_sql() -> None:
    ...
    with pytest.raises(ValueError, match="384"):
        await repository.search_semantic_candidates([0.0] * 383, 5, KnowledgeScope.INTERNAL)
    with pytest.raises(ValueError, match="between 1 and 10"):
        await repository.search_semantic_candidates([0.0] * 384, 0, KnowledgeScope.INTERNAL)
