            ...
            with pytest.raises(_RollbackScenario):
                async with database.transaction() as connection:
                    ...
                    lexical = await repository.search_lexical_candidates(
                        active_token, 10, KnowledgeScope.INTERNAL
                    )
                    assert lexical
                    ...
                    semantic = await repository.search_semantic_candidates(
                        active_embedding, 10, KnowledgeScope.INTERNAL
                    )
                    assert semantic
                    ...
                    assert (
                        await repository.search_lexical_candidates(inactive_token, 10, KnowledgeScope.INTERNAL)
                    ) == ()
                    ...
                    assert (
                        await repository.search_lexical_candidates(
                            document_inactive_token, 10, KnowledgeScope.INTERNAL
                        )
                    ) == ()
