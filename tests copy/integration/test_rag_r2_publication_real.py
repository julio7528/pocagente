        ...
        try:
            ...
            async with database.connection() as connection:
                ...
                for query in (
                    ...
                    lexical = await repository.search_lexical_candidates(query, 10, KnowledgeScope.INTERNAL)
                    semantic = await repository.search_semantic_candidates(
                        np.asarray(adapter.embed_query(query), dtype=np.float32), 10, KnowledgeScope.INTERNAL
                    )
