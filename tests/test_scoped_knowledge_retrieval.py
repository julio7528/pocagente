...
def test_persistent_hybrid_retrieval_rejects_none_scope() -> None:
    repository = MagicMock()
    adapter = MagicMock()
    adapter.embed_query.return_value = [0.0] * 384
    retriever = HybridRetriever(LexicalRetriever(repository), SemanticRetriever(repository, adapter))

    with pytest.raises(ValueError, match="persistent retrieval"):
        asyncio.run(retriever.search("question", KnowledgeScope.NONE))
